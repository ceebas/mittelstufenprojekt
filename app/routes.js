...
module.exports = function(app, passport, fs, multiparty, bcrypt, mysql) {
	...
	mysql.connect(db.connection.host, function(err, client, done) {
		...
		app.get('/', function(request, response) {
			client.query("select * from " + db.tableGames + " WHERE inactive = 0", function(err, rows, fields) {
				if (err) {
		...
		app.get('/play', function(request, response) {
			...
			client.query("select * from " + db.tableGames + " WHERE id_game LIKE '" + gameId + "' AND inactive = 0", function(err, rowsGame, fields) {
				if (err) {
					console.log(JSON.stringify(err));
					...
					client.query("SELECT high.*, user.username FROM " + db.tableHighscores + " high left join " + db.tableUsers + " user ON high.user = user.id_user WHERE game LIKE '" + gameId + "'ORDER BY score DESC LIMIT 10", function(err, rowsScore, fileds) {
						if (err) {
							console.log(err);
						}
		...
		app.post('/search', function(request, response) {
			...
			if (searchString == "" || searchString == "'" || searchString == " ") {
				...
				client.query("SELECT * FROM " + db.tableGames + " WHERE (gamename LIKE '%" + searchString + "%' or description LIKE '%" + searchString + "%')", function(err, rows, fields) {
					if (err) {
						console.log(JSON.stringify(err));
					}
					response.render('index.jade', {
		...
		app.get('/userSetting.html',isLoggedIn, function(request, response) {
			client.query("select * from " + db.tableGames +" WHERE user='" + request.user.id_user +"'", function(err, rows, fields) {
				if (err) {
					console.log(JSON.stringify(err));
				}
		...
		app.post('/submitScore', function(request, response) {
			if (request.user != undefined) {
				...
				client.query("INSERT INTO " + db.tableHighscores + " (user, game, score, tStamp) VALUES (?, ?, ?, NOW())", [request.user.id_user, gameData.gameId, gameData.score], function(err, rows, fields) {
					if (err) {
						console.log(JSON.stringify(err));
		...
		app.post('/userSetting', isLoggedIn, function(request,response) {
			...
			if (newData.passwordControl != newData.newPassword) {
				...
				client.query("SELECT * FROM " + db.tableUsers + " WHERE username='" + newData.id_user + "'", function(err, rows, fields) {
					if (err) {
						request.flash('message', err);
					} else {
						client.query(sqlUpdate, function(err, rows, fields) {
							if (err) {
								console.log(JSON.stringify(err));
								request.flash('message', err);
		...
		app.post('/uploads',isLoggedIn, function(req, res) {
	    	...
	    	form.parse(req, function(err, fieldsObject, filesObject, fieldsList, filesList) {
	    		...
	    		client.query("INSERT INTO " + db.tableGames + "(gamename, description, user, inactive) VALUES (?, ?, ?, 0)", [fieldsObject.gamename, fieldsObject.gamedescription, req.user.id_user], function(err, rows, fields) {			
	    			if (!err) {
						// Gibt die ID des des zufor erstellten Datensatzes aus
						client.query("SELECT LAST_INSERT_ID() as id_game", function(err, rows, fields){
							if (!err) {
										...
										if (err) {
											...
											for (var i = 0; i < filesObject.datei.length; i++) {
												...
												if (fileName == "image") {
													...
													client.query("UPDATE " + db.tableGames + " SET imageEnc='" + fileEnc + "' WHERE id_game='" + gameId + "'", function(err, rows, fields) {
														if (err) {
															console.log(JSON.stringify(err));
														}
													});
												...
												//JS Datei wird ebenfalls in Dtansatz gebracht
													...
													client.query("UPDATE " + db.tableGames + " SET javascript='" + originalFilename + "' WHERE id_game='" + gameId + "'", function(err, rows, fields) {
														if (err) {
															console.log(JSON.stringify(err));
														}
													});
		...
		app.get('/tableUsers.html', isLoggedIn, function(request, response) {
			if (request.user.isAdmin == 1) {
				client.query("select * from " + db.tableUsers + " WHERE isAdmin NOT LIKE 1", function(err, rows, fields) {
					if (err) {
						console.log(JSON.stringify(err));
						response.redirect('/');
					} else {
		...
		app.get('/tableGames.html', isLoggedIn, function(request, response) {
			if (request.user.isAdmin == 1) {
				client.query("select * from " + db.tableGames, function(err, rows, fields) {
					if (err) {
						console.log(JSON.stringify(err));
					}
		...
		app.get('/editUser.html', isLoggedIn, function(request, response) {
			if (request.user.isAdmin == 1) {
				...
				client.query("select * from " + db.tableUsers +" WHERE id_user='" + editId +"'", function(err, rows, fields) {
		...
		app.get('/removeGame', isLoggedIn, function(request, response) {
			if (request.user.isAdmin == 1) {
				...
				client.query("SELECT * FROM " + db.tableGames + " WHERE id_game=" + gameId, function(err, rows, fields) {
					if (err) {
						console.log(JSON.stringify(err));
						response.redirect('/');
					} else {
						...
						client.query("UPDATE " + db.tableGames + " SET inactive='" + inactive + "' WHERE id_game='" + gameId + "'", function(err) {
							if (err) {
								console.log(JSON.stringify(err));
							}
		...
		app.get('/removeUser', isLoggedIn, function(request, response) {
			if (request.user.isAdmin == 1) {
				...
				client.query("SELECT * FROM " + db.tableUsers + " WHERE id_user=" + userId, function(err, rows, fields) {
					if (err) {
						console.log(JSON.stringify(err));
						response.redirect('/');
					} else {
						...
						client.query("UPDATE " + db.tableUsers + " SET inactive='" + inactive + "' WHERE id_user='" + userId + "'", function(err) {
							if (err) {
								console.log(JSON.stringify(err));
							}
							response.redirect('/tableUsers.html');
